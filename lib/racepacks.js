...
if (isBrowser) {
  ...
  try {
    ...
    redirectStream.once('finish', function () {
      redirectStreamData = true
    })
...
function Racepacks (setup, tableOutput = true, targetThis = null) {
  if (__objCounter > 0) {
  ...
  this.rotationalRunOrder = buildRotationalRunOrder(Object.keys(hotPlugSetup.packs))
  Object.freeze(this.rotationalRunOrder)

  // TODO redesign for accessibility, getters and setters etc
  this.raceTimes = []
  this.raceDescriptionTable = []
  this.raceStats = {}

  race.call(this, tableOutput)
  return this
}
...
const racepacksSetupConsoles = function (redirectStream, isBrowser) {
  if (!isBrowser) {
    if (redirectStream) {
      // any output from tests will go to a custom stream
      // this prevents messing with racepacks output
...
Racepacks.prototype.raceSummary = function (raceStats) {
  if (typeof raceStats !== 'object') {
    throw new TypeError('raceSummary: invalid member "raceStats"')
  }
  ...
  for (const packName in raceStats) {
    raceStatsTable.push({
      ...
      'Efficiency (% Last)': '0 %',
      'Efficiency (% Next)': '0 %',
      'Total tests time': Number(raceStats[packName].ttime),
  ...
  for (let i = 0; i < rstLen; i++) {
    ...
    raceStatsTable[i]['Efficiency (% Last)'] =
      (raceStatsTable[rstLen - 1]['Total tests time'] * 100 / raceStatsTable[i]['Total tests' +
      ' time'] - 100).toFixed(2) + ' %'
    ...
    raceStatsTable[i]['Efficiency (% Next)'] =
      (raceStatsTable[effNext]['Total tests time'] * 100 / raceStatsTable[i]['Total tests' +
      ' time'] - 100).toFixed(2) + ' %'
  }
...
// TODO add tests for all
Racepacks.prototype.describeRace = function (raceTimes, raceDescriptionTable, tableOutput = true) {
...
const race = function (tableOutput = true) {
  this.raceTimes = []
  this.raceDescriptionTable = []
  this.raceStats = {}
  ...
  this.describeRace(this.raceTimes, this.raceDescriptionTable, tableOutput)
  this.raceSummary(this.raceStats)
